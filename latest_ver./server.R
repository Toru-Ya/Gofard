...
###Data_tab-----------------
  ...
  output$scatter_plot <- renderPlot({
    if (input$plot_group == "OFF") {
      if (input$scatter_line == "OFF") { 
        plot(inputfile()[, c(input$plot_x, input$plot_y)],
             pch = 16)
      } else {
        plot(inputfile()[, c(input$plot_x, input$plot_y)],
             pch = 16)
        lm.obj<-lm(inputfile()[,input$plot_y]~inputfile()[,input$plot_x])
        abline(lm.obj)
      }
    } else if (input$plot_group != "OFF")  {
      plotgroup=as.factor(inputfile()[,input$plot_group])
      num_plotgroup=length(unique(plotgroup))
      splitdata_scatter=split(inputfile(), inputfile()[,input$plot_group])
      if (input$scatter_line == "OFF") {
        plot(inputfile()[, c(input$plot_x, input$plot_y)]
  ...
  observeEvent(input$trigger_barbox, {#barbox plot swich start
    output$bar_plot <- renderPlot({
      if (input$bar_scatter == "OFF") {
        bar_plot=ggplot(inputfile_barbox(),
    ...
    output$box_plot <- renderPlot({
      box_plot=ggplot(inputfile_barbox(),
                      aes_string(x=input$data_barbox_x,
                                 y= input$data_barbox_y,
  ...
  observeEvent(input$trigger_histogram, {#histogram swich start
    output$histogram_plot <- renderPlot({
      if (input$data_histogram_color=="OFF"){
        histogram_plot=ggplot(inputfile_histogram(),
  ...
  observeEvent(input$trigger_corrplot, {#corrplot swich start
    output$scatter_matrix <- renderPlot({
      col=colnames(inputfile_corrplot()[,input$data_corrplot]) %>%
        paste(collapse = "+")
      formula_mat=paste0("~",col) %>%
        as.formula() 
      scatterplotMatrix(formula=formula_mat,
                        data=na.omit(inputfile_corrplot()[,input$data_corrplot]))
      
    })
    output$corrplot_plot <- renderPlot({
      cor_numVar=scale(inputfile_corrplot()[,input$data_corrplot])%>%
...
###prediction_tab-----------
  ...
  observeEvent(input$regression_button_lslr, {#pred swich start
    ...
    data_insp_lslr <- reactive({
      est.Y <- lslrcoef()[[1]]+lslrcoef()[[2]]*inputfile_reg_lslr()[,input$data_reg_x_lslr]
      residuals=inputfile_reg_lslr()[,input$data_reg_x_lslr]-est.Y
  ...
  　#prediction dataout tab
    ...
    data_insp_lasso <- reactive({
      prediction_data <- predict(Best.lasso(), 
                                 newx = as.matrix(inputfile_reg_lasso()[,input$data_reg_x_lasso]))
      residuals=inputfile_reg_lasso()[,input$data_reg_y_lasso]-prediction_data
