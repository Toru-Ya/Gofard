...
###Data_tab-----------------
  ...
  output$data_barbox_x = renderUI({ 
    selectInput("data_barbox_x", "x軸(カテゴリデータ)を選択", colnames(inputfile_barbox()))
  })
  output$data_barbox_y = renderUI({ 
    selectInput("data_barbox_y", "y軸を選択", colnames(inputfile_barbox()))
  })
...
###Classification tab-------
  ...
  output$zzzz=renderPrint({
    ...
    #print(class(data))
    #print(data)
    print(aes_string(x = input$plot_x, y = input$plot_y))
