...
shinyUI(navbarPage(theme = shinytheme("sandstone"),"Gofard",
      ...
      ##Home tab-------
       ...
       #a(href="https://github.com/Toru-Ya/MEIX", 
        # p("https://github.com/Toru-Ya/MEIX")),
       p("GofardはPCのローカル環境での使用を推奨します。インストールガイドは下記URLから。"),
       #a(href="http://gofard.com/install/", 
      #   p("http://gofard.com/install/")),
       helpText("©️ 2022 Toru Yashima"),
       h2("________________________________________________"),
       h2("Gofard ver.1.0 Menu"),
       h3("■データハンドリング"),
       p("散布図：データの散布図を作成し、データの全体像を観察できます。"),
       ...
       p("教師無し分類（k-mean）：k-mean法にて近い性質のデータサンプルごとに分類を行えます。"),
       h2("________________________________________________"),
       ),
       ...
       navlistPanel("Gofardでのデータ分析",
         tabPanel("基本の分析用データの作り方",
                  ...
                  p("例：散布図でのデータ確認"),
                  img(src="Gofard1.0_h2_jp.png", height = 500, width = 600),
                  h3("表データ作成の注意点"),
         ...
         tabPanel("実験計画表(DSD)の使い方",
                  ...
                  p("下図の「実験条件表」を作成し、MEIXに読み込めばワンクリックでスクリーニング計画表を出力します。"),
         ...
         tabPanel("単回帰分析(最小二乗法)の説明",
                  ...
                  p("予測モデル結果タブで予測モデル式やモデルの精度確認、
                    リストでの実測値とモデルの計算値との比較が行えます。またこの式を検量線とも言います。"),
      ...
      tabPanel("散布図", sidebarLayout(
        sidebarPanel(
          ...
          htmlOutput("colname1"),
          htmlOutput("colname2"),
          h5("3.グラフ作成_図をクリックしてタブを確認"),
          actionButton("trigger_scatter_plot", "プロットを出力"),
          ),
          mainPanel(
      ...
      tabPanel("実験計画表作成(DSD)", sidebarLayout(
        ...
        mainPanel(
          tabsetPanel(type = "tabs",
                      ...
                      tabPanel("実験計画表出力", 
                               h4("実験計画表"),
                               #h6("学習用データに目的変数の予測値を結合させた表データを返します"),
     ...
     tabPanel("決定木", sidebarLayout(
       sidebarPanel(
         ...
         #h5("4.予測用データcsvファイルを選択"),
         #uif_csv_uplode("datafile_pred_tree", ""),
